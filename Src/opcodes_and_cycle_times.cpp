...
enum EFlag
{
	N = 0 , // negative flag (1 when result is negative)
	V = 1 , // overflow flag (1 on signed overflow)
	x = 2 , // unused (always 1)
	B = 3 , // break flag (1 when interupt was caused by a BRK)
	D = 4 , // decimal flag (1 when CPU in BCD mode)
	I = 5 , // IRQ flag (when 1"," no interupts will occur (exceptions are IRQs forced by BRK and NMIs))"
	Z = 6 , // zero flag (1 when all bits of a result are 0)
	C = 7 , // carry flag (1 on unsigned overflow)
	_COUNT
};
...
struct CommandInfo
{
	...
	EFlagSetMode		m_flagMode[ EFlag::_COUNT ];
};
...
void BuildOpcodeTables()
{
	...
	for ( int i = 0 ; i < 77 * 21; i+= 21 )
	{
		...
		{
			int opcodeWithEAIndex = std::stoi( g_fullCommandList[ i + ea + 1 ], nullptr, 0 ); 
			CommandInfo& command = g_commands[ ea ];
